...
object PluginDeps {
    ...
    val PluginMicronaut = DepPlugin("Micronaut", "io.micronaut.application", version = "3.3.2").also { it.add() }
...
object Deps {
    ...
    val Antlr            = Dep.from("org.antlr:antlr4:4.9.3").also { it.add(APACHE_GROUP) }
    ...
    val Ktor             = Dep.from("io.ktor:ktor-server-core:1.6.8").also { it.add(WEB_GROUP) }
    ...
    val KotlinxYaml      = Dep.from("net.mamoe.yamlkt:yamlkt:0.10.2").also { it.add(SERIALIZATION_GROUP) }
    val KotlinxKaml      = Dep.from("com.charleskorn.kaml:kaml:0.43.0").also { it.add(SERIALIZATION_GROUP) }
    ...
    val Kotest = Dep.from("io.kotest:kotest-runner-junit5-jvm:5.2.2").also { it.add(TESTING_GROUP) }
