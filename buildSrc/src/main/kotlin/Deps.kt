...
object Deps {
    ...
    class Plugins { companion object {
        val Micronaut = DepPlugin("Micronaut", "io.micronaut.application", VERSION = "3.3.2").also { it.add() }
    ...
    class Tests { companion object {
        ...
        val Kotest = Dep.from("io.kotest:kotest-runner-junit5-jvm:5.2.2").also { add(it, Tests) }
    }}

    class JetBrains { companion object {
        var VERSION_Kotlin = UNSPECIFIED
        val Kotlin = Dep.from("org.jetbrains.kotlin:kotlin-gradle-plugin:1.6.20").also { VERSION_Kotlin = it.version ; add(it, JetBrains) }
        ...
        val Compose = Dep.from("org.jetbrains.compose:compose-gradle-plugin:1.1.1", repo = JETBRAINS).also { add(it, JetBrains) }
    ...
    class Micronaut { companion object {
        ...
        val Bom = Dep.from("io.micronaut:micronaut-bom:3.4.1").also { VERSION_Micronaut = it.version ; add(it, Micronaut) }
    }}

    class DB { companion object {
        val Postgresql = Dep.from("org.postgresql:postgresql:42.3.3", versionRegex = "${THREEDIGITSs}\\.jre\\d*\$").also { add(it, DB) }
        val H2 = Dep.from("com.h2database:h2:2.1.210").also { add(it, DB) }
    }}
    ...
    class Logging { companion object {
        ...
        val Logback = Dep.from("ch.qos.logback:logback-classic:1.2.11").also { VERSION_Logback = it.version ; add(it, Logging) }
        var VERSION_Slf4j = UNSPECIFIED
        val Slf4jApi = Dep.from("org.slf4j:slf4j-api:1.7.36").also { VERSION_Slf4j = it.version ; add(it, Logging) }
