...
object PluginDeps {
    ...
    val PluginMicronaut = DepPlugin("Micronaut", "io.micronaut.application", version = "3.7.0").also { it.add() }
...
object Deps {
    ...
    val Sslcontext       = Dep.from("io.github.hakky54:sslcontext-kickstart:7.4.9").also { it.add(WEB_GROUP) }
    val UUID             = Dep.from("com.benasher44:uuid:0.6.0").also { it.add(MPP_GROUP) }

    // SERIALIZATION
    val KotlinxJson      = Dep.from("org.jetbrains.kotlinx:kotlinx-serialization-json:1.5.0-RC").also { it.add(SERIALIZATION_GROUP) }
    val KotlinxYaml      = Dep.from("net.mamoe.yamlkt:yamlkt:0.12.0").also { it.add(SERIALIZATION_GROUP) }
    val KotlinxKaml      = Dep.from("com.charleskorn.kaml:kaml:0.51.0").also { it.add(SERIALIZATION_GROUP) }
    ...
    val Okio             = Dep.from("com.squareup.okio:okio:3.3.0").also { it.add(SQUAREUP_GROUP) }
    val Retrofit         = Dep.from("com.squareup.retrofit2:retrofit:2.9.0").also { it.add(SQUAREUP_GROUP) }
    val SQLDelight       = Dep.from("com.squareup.sqldelight:gradle-plugin:1.5.5").also { it.add(SQUAREUP_GROUP) }
    ...
    // TESTING
    val Junit = Dep.from("org.junit.jupiter:junit-jupiter-api:5.9.2").also { it.add(TESTING_GROUP) }
